...
from networkconfparse import ConfigNode, parse


def test_empty_config_yields_bare_root() -> None:
    """An empty string parses to a childless root node."""
    root = parse("")
    assert root.is_root
    assert root.children == []


def test_top_level_lines_are_root_children() -> None:
    """Unindented lines attach directly to the root."""
    root = parse("hostname r1\nip routing\n")
    assert [c.text for c in root.children] == ["hostname r1", "ip routing"]
...
def test_deep_nesting() -> None:
    ...
    root = parse(cfg)
    police = root.children[0].children[0].children[0]
...
def test_dedent_returns_to_correct_parent() -> None:
    ...
    assert root.children[0].children[0].text == "ip address 10.0.0.1 255.255.255.0"
    assert root.children[1].children[0].text == "shutdown"
...
def test_tabs_are_expanded() -> None:
    ...
    cfg = "interface Gi0/0\n\tshutdown\n"
    root = parse(cfg)
    assert root.children[0].children[0].text == "shutdown"
